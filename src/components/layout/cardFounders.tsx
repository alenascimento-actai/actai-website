...
export const FounderCard = ({
  ...
  return (
    <div className="p-6 rounded-2xl flex flex-col md:flex-row gap-16 items-center w-full backdrop-blur-sm bg-white/20">
      ...
      <div className="flex flex-col justify-between items-baseline text-white text-sm h-[334px]">
        <div className="flex items-center justify-between w-full">
          <h3 className="text-xl font-semibold mb-2 text-white">{name}</h3>
          <span className="text-xs max-w-52 w-full py-3 rounded-2xl border border-white text-white text-center font-semibold">
            {title}
          </span>
        </div>
        <p className="text-white/80 leading-relaxed text-sm">{bio}</p>
      </div>
    </div>
  );
};
